...
export default function DocsPage() {
  return (
    <main className="min-h-screen overflow-x-clip bg-[#0a0d10] text-[#f4f0e8]">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
        ...
        <section className="max-w-3xl py-16 sm:py-20">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[#e29a63]/20 bg-[#e29a63]/[.08] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#f2b27b]"><Sparkles className="size-3.5" /> Studio handbook</div>
          <h1 className="text-4xl font-semibold tracking-[-0.04em] text-[#f4f0e8] sm:text-6xl">Make ideas tangible.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-white/55">A practical guide to the canvas editor: create quickly, refine confidently, and keep every version safely persisted through the Express and MongoDB stack.</p>
        </section>

        <section className="grid gap-4 md:grid-cols-2">
          {sections.map(({ icon: Icon, title, body }) => (
            <article key={title} className="rounded-2xl border border-white/10 bg-[#15191d] p-6 shadow-[0_18px_50px_rgba(0,0,0,.14)] transition hover:-translate-y-0.5 hover:border-[#e29a63]/25 hover:bg-[#1a2028]">
              <div className="mb-5 grid size-10 place-items-center rounded-xl border border-[#e29a63]/20 bg-[#e29a63]/[.09] text-[#f2b27b]"><Icon className="size-4" /></div>
              <h2 className="text-lg font-semibold text-[#f4f0e8]">{title}</h2>
        ...
        <section className="mt-16 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <article className="rounded-2xl border border-white/10 bg-[#15191d] p-6 sm:p-8">
            <div className="mb-6 flex items-center gap-3"><Keyboard className="size-5 text-[#f2b27b]" /><h2 className="text-xl font-semibold text-[#f4f0e8]">Keyboard shortcuts</h2></div>
            <div className="divide-y divide-white/8">{shortcuts.map(([key, label]) => <div key={key} className="flex items-center justify-between gap-4 py-3 text-sm"><span className="text-white/50">{label}</span><kbd className="rounded-md border border-white/15 bg-[#0a0d10] px-2 py-1 font-mono text-[11px] text-[#f2b27b]">{key}</kbd></div>)}</div>
          </article>
          <article className="rounded-2xl border border-[#e29a63]/20 bg-[linear-gradient(145deg,rgba(226,154,99,.12),rgba(21,25,29,.72))] p-6 sm:p-8">
            <div className="mb-6 flex items-center gap-3"><Server className="size-5 text-[#f2b27b]" /><h2 className="text-xl font-semibold text-[#f4f0e8]">Built for safe persistence</h2></div>
        ...
        <footer className="mt-20 border-t border-white/10 pt-6 text-xs text-white/35"><Link href="/" className="transition hover:text-[#f2b27b]">Design Canvas</Link><span className="mx-2 text-white/20">/</span> Documentation · Built for focused visual thinking</footer>
      </div>
    </main>
  )
}
